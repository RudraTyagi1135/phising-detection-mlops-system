...
def main() -> int:
    ...
    if settings.data.target_column not in schema_column_names:
        _fail(f"Target column {settings.data.target_column} is missing from schema")

    required_paths = [
        settings.paths.templates_dir,
        settings.paths.schema_file_path,
        settings.paths.local_dataset_path,
    ]
    ...
    print(f"Project validation passed. MongoDB health: {health['status']}")
    return 0
